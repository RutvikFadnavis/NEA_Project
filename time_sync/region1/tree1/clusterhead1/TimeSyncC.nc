...
#include "printf.h"
 ...
 module TimeSyncC {
  ...
  bool busy = FALSE;
  message_t pkt;
  uint32_t t1=0,t2=0,t3=0,t4=0,nodeid=111,done=0,T1=0,T2=0,T3=0,T4=0,
           propogation_delay=0,phase_drift=0,propogation_delay_1=0,phase_drift_1=0,to_send=1111,timesynced=0;
...
/*-----------------------------------------time sync starts------------------------------------------------------*/
	...
	//MotetoMoteMsg* newpkt = (MotetoMoteMsg*)(call Packet.getPayload(&pkt, sizeof(MotetoMoteMsg)));	
		...
		else if(!busy && ((T2+T3+T4)==0) && (done==1))
	...
	//start of t4 condition n final transmission to next level
			else if(!busy && T1>0 && T2>0 && T3>0 && T4>0  && done==1 && to_send<=1112){
			MotetoMoteMsg* newpkt = (MotetoMoteMsg*)(call Packet.getPayload(&pkt, sizeof(MotetoMoteMsg)));
			newpkt-> t1 = T1;
			newpkt-> t2 = T2;
			newpkt-> t3 = T3;
